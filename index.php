<?php
  ...
  {
    $_SESSION['file'] = "https://www.google.fr/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png";
	}
   $_SESSION['live'] = "e";
	...
	{
		echo "<span class='msg msg-success'>Votre image a ete envoyé avec succès</span>";
		$_SESSION['success'] = "KO";
	}
...
<div class="where">
	...
	<span class="ou">Prendre avec sa webcam</span>
</div>
<video id="video" width="976" height="732" autoplay></video>

<button id="snap" class ="btn btn-default btn-center">Prendre la photo</button>

<canvas id="canvas1" class="hidden" width="976" height="732" ondrop="drop(event)" ondragover="enableDrop(event)" autoplay></canvas>
<script type="text/javascript" src="js/cam.js"></script> 
<hr class="fancy">
<div class="where">

	<span class="ou">Ou envoyer un fichier</span>
</div>
<form enctype="multipart/form-data" action="index.php#canvas" method="post">
  <!-- MAX_FILE_SIZE doit précéder le champ input de type file -->
  <input type="hidden" name="MAX_FILE_SIZE" value="999999999" />
  <!-- Le nom de l'élément input détermine le nom dans le tableau $_FILES -->
 <p><label for="userfile"> Envoyez un fichier :</label> <br><br><input name="userfile" type="file" class="file" />
 </p>
  <input type="submit" value="Envoyer le fichier" name="file" class="btn btn-default btn-center btn-orange"	/>
</form>
...
<hr class="fancy">
  <div class="part">
    <div class="camagru">
      <h2>Ton montage</h2>
      <hr class="fancy"/>
<?php if ($_SESSION['file'] != "https://www.google.fr/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png") { ?>
<canvas id="canvas" class="cadre floatl" width="640" height="480" ondrop="drop(event)" ondragover="enableDrop(event)" autoplay></canvas>
<?php } else {?>

<canvas id="canvas" class="cadre floatl hidden" width="640" height="480" ondrop="drop(event)" ondragover="enableDrop(event)" autoplay></canvas>
<canvas id="canvas2" class="cadre floatl" width="640" height="480" ondrop="drop(event)" ondragover="enableDrop(event)" autoplay></canvas>
<?php } ?>
<div class="render">
<div class="image">
  <?php
...
<?php } else {?>
<button id="save" class="btn btn-default btn-center hidden">Save</button>
<?php } ?>
</div>
</div>

<div class="last">
  <h2>Mes dernieres creation</h2>
  <hr class="fancy"/>

  <?php
    $requete = $bdd->prepare("SELECT * FROM image WHERE id_users = ? ORDER BY id DESC LIMIT 6");
    $requete->execute(array($id_users));
...
if( filter_has_var( INPUT_POST, 'file' ) )
 {
 	if ($_FILES['userfile']['error'] > 0) {
 		...
 		echo "<p class='msg msg-errors'>$erreur</p>";
 	}
 	else
 	{
 		...
 		if ( in_array($extension_upload,$extensions_valides) ) 
 		{
			...
			imagecopyresampled($tmp,$src,0,0,0,0,$newwidth,$newheight, $width,$height);
 			$filename = "tmp/".time().rand().".".$extension_upload;
...
if (isset($GLOBALS["HTTP_RAW_POST_DATA"]) && isset($_GET['img']) && $_GET['img'] == 2)
{
   // Get the data
  $imageData=$GLOBALS['HTTP_RAW_POST_DATA'];
